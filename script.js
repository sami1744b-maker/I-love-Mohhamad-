...
// Get sender and recipient names from URL
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Page loaded, senderName:', senderName, 'recipientName:', recipientName);
    
    if (senderName) {
        // If there's a sender name in URL, show the result directly
        console.log('Showing result for sender:', senderName);
        showResult(senderName, recipientName);
    } else {
...
// Handle recipient name form submission
document.getElementById('recipientNameForm').addEventListener('submit', function(e) {
    ...
    if (recipientName) {
        ...
        const currentSender = getUrlParameter('sender') || document.getElementById('userName').value;
        
        // Create new shareable URL with both sender and recipient
        const shareUrl = `${window.location.origin}${window.location.pathname}?sender=${encodeURIComponent(currentSender)}&recipient=${encodeURIComponent(recipientName)}`;
        
        // Update URL without page reload
        window.history.pushState({}, '', shareUrl);
        
        // Update the display
        showResult(currentSender, recipientName);
        
        // Hide the recipient form since name is now added
        document.getElementById('recipientForm').style.display = 'none';
    }
});

function showResult(sender, recipient = null) {
    // Hide form
    ...
    // Update message based on whether there's a recipient
    if (recipient) {
        document.getElementById('senderMessage').textContent = `${sender} और ${recipient} ने मुहम्मद साहब पर दरूद भेजा है`;
    } else {
...
function shareWhatsApp() {
    ...
    if (senderName && recipientName) {
        message = `${senderName} और ${recipientName} ने मुहम्मद साहब पर दरूद भेजा है। आप भी दरूद भेजें: ${currentUrl}`;
    } else if (senderName) {
        message = `${senderName} ने मुहम्मद साहब पर दरूद भेजा है। आप भी दरूद भेजें: ${currentUrl}`;
    } else {
        message = `मुहम्मद साहब पर दरूद भेजें: ${currentUrl}`;
    }
